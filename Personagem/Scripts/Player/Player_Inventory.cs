...
public class Player_Inventory : MonoBehaviour
{
    ...
    void CheckIfHandsEmpty()
    {
        ...
        {
            StartCoroutine(PlaceItensInHands(listInventory[listInventory.Count - 1]));
        }
    }
    ...
    public void ActivateInventoryItem(int inventoryIndex)
    {
        ...
        DeactivateAllInventoryItens();
        StartCoroutine(PlaceItensInHands(listInventory[inventoryIndex]));
    }

    void DeactivateAllInventoryItens()
    {
        foreach(Transform child in inventoryPlayerParent)
        {
            if(child.CompareTag("Item"))
    ...
    IEnumerator PlaceItensInHands(Transform itemTransform)
    {
